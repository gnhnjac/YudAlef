...
class Network:
    ...
    def batch_gradient_descent(self, inputs, targets, learning_rate, loss_function, loss_function_derivative,
                               batch_size=32,
                               epochs=1, graph=False, verbose=False,
                               eval_function=lambda x, y: np.argmax(x) == np.argmax(y), xtest=None, ytest=None):

        for epoch in range(epochs):
            inpoutpairs_batch = list(zip(inputs, targets))
            np.random.shuffle(inpoutpairs_batch)
            inpoutpairs_batch = np.array_split(inpoutpairs_batch, len(inpoutpairs_batch) // batch_size)

            for batch in inpoutpairs_batch:
                weight_grad_cache = []
                bias_grad_cache = []

                for layer in self.layers:
                    if isinstance(layer, Convolutional) or isinstance(layer, Dense):
                        weight_grad_cache.append(np.zeros(layer.weight_matrix.shape))
                        bias_grad_cache.append(np.zeros(layer.bias_vector.shape))

                self.batch_descend(batch, learning_rate, loss_function, loss_function_derivative)

    def batch_descend(self, batch, lr, loss, d_loss, weight_cache, bias_cache):
        ...
        for inp, target in batch:
            ...
            output_gradient = d_loss(inp, target)

            for layer in reversed(self.layers):
                output_gradient = layer.back_propagate(output_gradient, lr)
